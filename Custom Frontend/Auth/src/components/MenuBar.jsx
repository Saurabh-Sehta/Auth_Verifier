...
const MenuBar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      const respoonse = await axios.post(`${backendURL}/logout`);
  ...
  const sendVerificationOtp = async () => {
    try {     
      const response = await axios.post(`${backendURL}/send-otp`, {}, axios.defaults.withCredentials = true);
  ...
  return (
    <nav className='navbar bg-white px-5 py-4 d-flex justify-content-between align-items-center'>
      ...
      {userData ? (<div className='position-relative' ref={dropdownRef}>
